...
<div class="row mb-3">
    ...
    <div class="col-md-6 text-right">
        ...
        <a href="{{ base_url('koordinator-pkl/cetak/mahasiswa_pkl') }}" target="blank" class="btn btn-danger" id="btn-export">Export PDF</a>
    </div>
</div>
                ...
                <tbody>
                    @foreach ($data as $v)
                        <tr>
                            ...
                            <td>{!! $v->nama_dosen? $v->nama_dosen : '<small class="text-muted">Belum ada pembimbing</small>' !!}</td>
                            ...
                            <td>
                                {{ $v->status == '1' ? $v->perusahaan : '-' }}
                            </td>
                            <td>

                            @if($v->status == '1')
                                <button class="btn btn-info" data-toggle="modal" data-target="#ModelEdit{{ $v->pembimbing_id }}">Edit</button>
                                @endif
                                @if($v->status == null)
                                <button class="btn btn-danger" data-toggle="modal" data-target="#ModalDelete{{ $v->pembimbing_id }}">Hapus</button>
                                @endif

                                <div class="modal fade text-left" id="ModelEdit{{ $v->pembimbing_id }}" tabindex="-1" role="dialog"
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <form action="{{ base_url('koordinator-pkl/mahasiswa/update') }}" method="post">
                                                <div class="modal-body">
                                                    <input type="hidden" name="id" value="{{ $v->mahasiswa_id }}">
                                ...
                                <div class="modal fade text-left" id="ModalDelete{{ $v->pembimbing_id }}" tabindex="-1" role="dialog"
                                    aria-labelledby="myModalLabel1" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-scrollable" role="document">
                                        <div class="modal-content">
                                            <div class="modal-header">
